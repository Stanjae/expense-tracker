...
import {  SignOutWithGoogle } from '../Hooks/useSigninOut';
...
const SideNav = () => {
    const history = useMatches()[1].pathname;
  ...
  return (
    <div className=" px-3 py-4 ">
      ...
      {Navs.map((item, index) => (
        <li key={index} className={' py-1.5  hover:bg-[#00B0FF]' + (item.url !== history ? '':' bg-[#00B0FF]')}>
            <Link to={item.url} className={" hover:text-white flex items-center p-2 rounded-lg  group" + (item.url !== history ? ' text-gray-900':' text-white')}>
               {item.icon}
               <span className="ml-3 hidden lg:block hover:text-white">{item.name}</span>
            </Link>
         </li>
      ))}   
      </ul>
      <div className=' my-5'>
            <button onClick={signOut} className="flex justify-start items-center w-full py-3 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
               <svg className="flex-shrink-0 w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 16">
                  <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M1 8h11m0 0L8 4m4 4-4 4m4-11h3a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2h-3"/>
               </svg>
               <span className=" hidden lg:block ml-3 whitespace-nowrap">Sign Out</span>
            </button>
        </div>
   </div>
  )
